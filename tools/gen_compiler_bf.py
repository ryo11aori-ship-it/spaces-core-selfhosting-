#!/usr/bin/env python3
import sys

# === Flat Helper Functions ===
S = " "
F = "\u3000"
def e(s): sys.stdout.write(s + "\n")
def R(n=1): 
    if n > 0: e((S + S + S) * n)
def L(n=1): 
    if n > 0: e((S + S + F) * n)
def I(n=1): 
    if n > 0: e((S + F + S) * n)
def D(n=1): 
    if n > 0: e((S + F + F) * n)
def O(): e(F + S + S)
def N(): e(F + S + F)
def B(): e(F + F + S)
def C(): e(F + F + F)
def Z(): B(); D(); C()

def emit_byte_to_buffer(val):
    Z()
    I(val)
    R(1)

def emit_bytes(vals):
    for v in vals:
        emit_byte_to_buffer(v)

def main():
    R(300)
    # ELF Header
    emit_bytes([0x7f, 0x45, 0x4c, 0x46, 0x02, 0x01, 0x01, 0x00, 0,0,0,0,0,0,0,0])
    emit_bytes([0x02, 0x00, 0x3e, 0x00, 0x01, 0x00, 0x00, 0x00])
    emit_bytes([0x78, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0,0,0,0,0,0,0,0])
    emit_bytes([0,0,0,0, 64,0, 56,0, 1,0, 64,0, 0,0, 0,0])
    emit_bytes([0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0,0,0,0,0,0,0,0])
    emit_bytes([0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0xa7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0xa7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    emit_bytes([0x48, 0xc7, 0xc3, 0x00, 0x20, 0x40, 0x00])
    
    # Read first char
    N()
    B()
    # Move Char from Ptr to Ptr+50
    Z(); I(1); L(1); B(); R(50); I(1); L(50); D(); C()
    R(50) 
    
    # === Identification ===
    # + (43) -> ID=1
    D(43)
    B(); R(1); D(); L(1); Z(); C() # Zero check
    R(1); B(); D(); R(1); I(1); L(2); C() # Set ID
    I(43) # Restore
    
    # , (44) -> ID=2
    D(44)
    B(); R(1); D(); L(1); Z(); C()
    R(1); B(); D(); R(1); I(2); L(2); C()
    I(44)
    
    # - (45) -> ID=3
    D(45)
    B(); R(1); D(); L(1); Z(); C()
    R(1); B(); D(); R(1); I(3); L(2); C()
    I(45)
    
    # . (46) -> ID=4
    D(46)
    B(); R(1); D(); L(1); Z(); C()
    R(1); B(); D(); R(1); I(4); L(2); C()
    I(46)
    
    # < (60) -> ID=5
    D(60)
    B(); R(1); D(); L(1); Z(); C()
    R(1); B(); D(); R(1); I(5); L(2); C()
    I(60)
    
    # > (62) -> ID=6
    D(62)
    B(); R(1); D(); L(1); Z(); C()
    R(1); B(); D(); R(1); I(6); L(2); C()
    I(62)
    
    # Cleanup Ptr+50, Ptr+51. Move to Ptr+52 (ID)
    Z(); R(1); Z(); R(1)
    
    # === Emission Chain ===
    # We are at ID. Flag is at ID+1 (Ptr+53).
    
    # ID=1 (+)
    D(1)
    R(1); Z(); I(1); L(1) # Flag=1
    B(); R(1); D(); L(1); C() # If ID!=0, Flag=0
    R(1) # At Flag
    B()
        D()
        L(53) # To Ptr
        I(0xfe); R(1); I(0x03); R(1)
        R(52) # To New ID
    C()
    
    # ID=2 (,)
    D(1)
    L(1); R(1); Z(); I(1); L(1) # Flag=1. (Need L(1) to go back to ID from Flag? No, we are at Flag. L(1) to ID)
    B(); R(1); D(); L(1); C()
    R(1)
    B()
        D()
        L(53)
        emit_bytes([0x48, 0xc7, 0xc0, 0x00, 0x00, 0x00, 0x00])
        emit_bytes([0x48, 0xc7, 0xc7, 0x00, 0x00, 0x00, 0x00])
        emit_bytes([0x48, 0x89, 0xde])
        emit_bytes([0x48, 0xc7, 0xc2, 0x01, 0x00, 0x00, 0x00])
        emit_bytes([0x0f, 0x05])
        R(52)
    C()
    
    # ID=3 (-)
    D(1)
    L(1); R(1); Z(); I(1); L(1)
    B(); R(1); D(); L(1); C()
    R(1)
    B()
        D()
        L(53)
        I(0xfe); R(1); I(0x0b); R(1)
        R(52)
    C()
    
    # ID=4 (.)
    D(1)
    L(1); R(1); Z(); I(1); L(1)
    B(); R(1); D(); L(1); C()
    R(1)
    B()
        D()
        L(53)
        emit_bytes([0x48, 0xc7, 0xc0, 0x01, 0x00, 0x00, 0x00])
        emit_bytes([0x48, 0xc7, 0xc7, 0x01, 0x00, 0x00, 0x00])
        emit_bytes([0x48, 0x89, 0xde])
        emit_bytes([0x48, 0xc7, 0xc2, 0x01, 0x00, 0x00, 0x00])
        emit_bytes([0x0f, 0x05])
        R(52)
    C()
    
    # ID=5 (<)
    D(1)
    L(1); R(1); Z(); I(1); L(1)
    B(); R(1); D(); L(1); C()
    R(1)
    B()
        D()
        L(53)
        emit_bytes([0x48, 0xff, 0xcb])
        R(52)
    C()
    
    # ID=6 (>)
    D(1)
    L(1); R(1); Z(); I(1); L(1)
    B(); R(1); D(); L(1); C()
    R(1)
    B()
        D()
        L(53)
        emit_bytes([0x48, 0xff, 0xc3])
        R(52)
    C()
    
    # End of Loop
    # We are at Flag (Ptr+53).
    # Clear Flag, Clear ID (Ptr+52).
    Z(); L(1); Z()
    
    # Return to Ptr
    L(52)
    
    # Read Next
    N()
    C()
    
    # Exit Syscall
    emit_bytes([0x48, 0xc7, 0xc0, 0x3c, 0x00, 0x00, 0x00])
    emit_bytes([0x48, 0x31, 0xff])
    emit_bytes([0x0f, 0x05])
    
    # Output Dump (Print 5000 bytes)
    L(1000); R(299); I(1); I(255)
    B(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); R(1); O(); D(); C()

if __name__ == "__main__":
    main()
